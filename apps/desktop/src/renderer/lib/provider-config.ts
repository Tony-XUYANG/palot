...
export function normalizeProviderBaseUrl(value: string): string {
	...
	if (!isHttps && !isLoopbackHttp) {
		throw new Error("Provider endpoints must use HTTPS. HTTP is allowed only for localhost.")
	}
	if (url.username || url.password) {
		throw new Error("Provider endpoint URLs cannot include credentials.")
	}
	if (url.search || url.hash) {
		throw new Error("Provider endpoint URLs cannot include a query string or fragment.")
	}
