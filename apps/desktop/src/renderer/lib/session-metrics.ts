...
export function computeSessionMetrics(messages: Message[]): SessionMetrics {
	...
	for (const msg of messages) {
		...
		// Model distribution
		if (msg.modelID) {
			modelDistribution[msg.modelID] =
				(modelDistribution[msg.modelID] ?? 0) + 1;
		}
	...
	const cacheEfficiency =
		totalInput > 0 ? (tokens.cacheRead / totalInput) * 100 : 0;
	...
	const exchangeCount =
		userIdsWithResponses.size > 0
			? userIdsWithResponses.size
			: userMessageCount;
		...
		return {
			...
			compactionPercentage:
				threshold != null && threshold > 0
					? Math.round((total / threshold) * 100)
					: null,
		};
	}
	return null;
