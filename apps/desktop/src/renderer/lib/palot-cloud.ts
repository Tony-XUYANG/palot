...
export async function applyPalotCloudProvider(setup: PalotCloudProviderSetup): Promise<void> {
	...
	const result = await client.global.config.update({
		config: {
			...
			provider: { [PALOT_CLOUD_PROVIDER_ID]: createPalotCloudProviderConfig(setup) },
		},
	})
	if (result.error) throw new Error("OpenCode rejected the Palot Cloud provider configuration")
...
export async function disablePalotCloudProvider(): Promise<void> {
	...
	if (result.error) throw new Error("OpenCode rejected the Palot Cloud provider update")
	await client.global.dispose()
}
