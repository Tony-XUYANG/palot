	...
	try {
		...
		if (state.displayMode)
			localStorage.setItem(
				"palot:displayMode",
				JSON.stringify(state.displayMode),
			);
		if (state.theme)
			localStorage.setItem("palot:theme", JSON.stringify(state.theme));
		if (state.colorScheme)
			localStorage.setItem(
				"palot:colorScheme",
				JSON.stringify(state.colorScheme),
			);
		if (state.drafts)
			localStorage.setItem("palot:drafts", JSON.stringify(state.drafts));
		if (state.projectModels)
			localStorage.setItem(
				"palot:projectModels",
				JSON.stringify(state.projectModels),
			);
 ...
 */
export const opaqueWindowsAtom = atomWithStorage<boolean>(
	"palot:opaqueWindows",
	false,
);
...
export const projectModelsAtom = atomWithStorage<
	Record<string, PersistedModelRef>
>("palot:projectModels", {});
