...
export const setAutomationsAtom = atom(
	null,
	(_get, set, automations: Automation[]) => {
		set(automationsAtom, automations);
	},
);

/** Write-only atom to set automation runs from a fetch. */
export const setAutomationRunsAtom = atom(
	null,
	(_get, set, runs: AutomationRun[]) => {
...
export const unreadRunCountAtom = atom((get) => {
	return get(automationRunsAtom).filter(
		(r) => r.readAt === null && r.status === "pending_review",
	).length;
});
...
export const markRunReadLocalAtom = atom(null, (get, set, runId: string) => {
	...
	const updated = runs.map((r) =>
		r.id === runId ? { ...r, readAt: Date.now() } : r,
	);
