...
function buildGlassVars(theme: ThemeDefinition): [string, string][] {
	...
	if (g.bodyOpacity !== undefined)
		vars.push(["--glass-body", `${g.bodyOpacity}%`]);
	if (g.sidebarOpacity !== undefined)
		vars.push(["--glass-sidebar", `${g.sidebarOpacity}%`]);
	if (g.surfaceOpacity !== undefined)
		vars.push(["--glass-surface", `${g.surfaceOpacity}%`]);
	if (g.elevatedOpacity !== undefined)
		vars.push(["--glass-elevated", `${g.elevatedOpacity}%`]);
	if (g.cardOpacity !== undefined)
		vars.push(["--glass-card", `${g.cardOpacity}%`]);
	if (g.contentOpacity !== undefined)
		vars.push(["--glass-content", `${g.contentOpacity}%`]);
...
export function useAvailableThemes(): ThemeDefinition[] {
	const platform =
		typeof window !== "undefined" && "palot" in window
