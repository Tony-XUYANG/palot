...
export function useServerActions() {
	const addServer = useCallback(
		async (server: RemoteServerConfig, password?: string) => {
			if (!isElectron) return;
	...
	const updateServer = useCallback(
		async (
			serverId: string,
			updates: Partial<RemoteServerConfig>,
				...
				return {
					...
					hasPassword:
						password === null
							...
							: password
								? true
								: (s as RemoteServerConfig).hasPassword,
				};
			});
	...
	const saveDiscoveredServer = useCallback(
		async (mdnsServer: DiscoveredMdnsServer, password?: string) => {
			...
			const resolvedHost =
				mdnsServer.addresses.find((a) => !a.includes(":")) || mdnsServer.host;
...
function triggerServerSwitch(newActiveServerId: string) {
	...
	const knownDirs = currentProjects
		.flatMap((p) => [p.worktree, ...(p.sandboxes ?? [])])
