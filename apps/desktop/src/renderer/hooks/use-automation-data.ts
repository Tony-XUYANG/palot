	...
	const refetch = useCallback(
		async (source = "unknown") => {
			...
			try {
					...
					if (a.status === "active") {
						log.debug("Active automation", {
							...
							nextRunAt: a.nextRunAt
								? new Date(a.nextRunAt).toISOString()
								: "none",
							lastRunAt: a.lastRunAt
								? new Date(a.lastRunAt).toISOString()
								: "never",
							runCount: a.runCount,
				...
				if (
					prevRunCountRef.current !== null &&
					runs.length !== prevRunCountRef.current
				) {
				...
				setError(msg);
				log.error("Failed to fetch automation data", {
					fetchId,
					source,
					error: msg,
				});
