...
function ScrollToResponseStart({
	...
	return (
		...
		>
			...
			<span>{useTranslation().t("chat.jumpToStart")}</span>
		</button>
	);
}
										...
										onForkFromTurn={
											onForkFromTurn
												? () => {
														...
														return onForkFromTurn(
															nextTurn?.userMessage.info.id,
														);
													}
												: undefined
										}
...
function ChatInputSection({
	...
	const effectivePermission = useAtomValue(
		effectivePermissionFamily(agent.sessionId),
	);
	const effectiveQuestion = useAtomValue(
		effectiveQuestionFamily(agent.sessionId),
	);
	...
	useEffect(() => {
		if (resetForSessionRef.current !== agent.sessionId) {
			...
			const stored = agent.directory
				? projectModels[agent.directory]
				: undefined;
	...
	const handleSend = useCallback(
		async (text: string, files?: FileAttachment[]) => {
			...
			try {
				...
				log.debug("handleSend onSendMessage completed", {
					sessionId: agent.sessionId,
				});
				clearDraft();
				setMentions([]);
				...
				toast.error("Could not send message", {
					description: formatRequestError(err),
				});
			} finally {
				setSending(false);
			}
	...
	const handleSlashTriggerChange = useCallback(
	...
	const handleMentionTriggerChange = useCallback(
		(open: boolean, query: string) => {
			setMentionOpen(open);
	...
	const handleMentionSelect = useCallback(
		(option: MentionOption) => {
			...
			const { text: newText, cursorPosition: newCursor } =
				insertMentionIntoText(currentText, cursorPos, mention);
			...
			setMentions((prev) => {
				...
				if (
					prev.some(
						(m) =>
							(m.type === "file" ? `file:${m.path}` : `agent:${m.name}`) ===
							key,
					)
				)
					return prev;
					...
					{isReverted && (
						<div className="mb-2 flex items-center gap-2 rounded-lg border border-amber-400/30 bg-amber-50 px-3 py-2 text-xs text-amber-800 dark:border-amber-500/20 dark:bg-amber-500/5 dark:text-amber-400">
							...
							<span className="flex-1">
								Session reverted — type to continue from here, or redo to
								restore
							</span>
							{canRedo && onRedo && (
								<button
					...
					{effectivePermission && (
						<div className="pb-2">
							<PermissionItem
								...
								isConnected={isConnected}
								isFromSubAgent={
									effectivePermission.sessionId !== agent.sessionId
								}
							/>
						</div>
					)}
								...
								>
									...
									<PromptInputFooter>
										...
										<PromptInputSubmit
											...
											onStop={handleStop}
											size={
												isWorking && currentTurnWorkSplit ? "xs" : "icon-sm"
											}
										>
...
function LiveTurnTimer({
	...
	const computeDisplay = useCallback(
		() =>
			formatWorkDuration(
				completedMs + (activeStartMs != null ? Date.now() - activeStartMs : 0),
			),
...
function WorktreeSetupProgress({
	phase,
}: {
	phase: NonNullable<SessionSetupPhase>;
}) {
	return (
