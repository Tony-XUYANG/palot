...
export function AutomationRunDetail() {
	...
	const needsSessionPoll =
		run !== null && run.status === "running" && !run.sessionId;
	...
	return (
		<RunStubView run={run} automationName={automation?.name ?? "Automation"} />
	);
}
...
function RunStubView({
	...
	const { t } = useTranslation();
	const duration =
		run.startedAt && run.completedAt ? run.completedAt - run.startedAt : null;
		...
		<div className="flex h-full flex-col items-center justify-center gap-4 p-8 text-center">
			...
			<div className="max-w-md space-y-2">
				<h3 className="text-sm font-semibold">
					{run.resultTitle ?? automationName}
				</h3>

				{duration !== null && duration > 0 && (
					<p className="text-xs text-muted-foreground">
						{t("automations.ranFor", { duration: formatDuration(duration) })}
					</p>
				)}
				...
				{run.resultSummary ? (
					...
					<p className="text-sm text-muted-foreground">
						{t("automations.startingSession")}
					</p>
				) : run.status === "queued" ? (
					<p className="text-sm text-muted-foreground">
						{t("automations.queuedDescription")}
					</p>
				) : run.status === "running" ? (
					<p className="text-sm text-muted-foreground">
