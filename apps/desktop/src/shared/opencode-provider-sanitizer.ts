...
export function sanitizeOpenCodeProviderPayload(path: string, value: unknown): unknown {
	if (!isRecord(value)) throw new Error("OpenCode provider response must be an object")
...
export function sanitizeOpenCodeProviderResponse(
	...
	if (path !== PROVIDER_CATALOG_PATH && path !== CONFIG_PROVIDERS_PATH) return body
