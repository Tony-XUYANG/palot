...
const provider = {
	...
	options: { apiKey: "secret-option-key", baseURL: "https://api.openai.com/v1" },
	models: {
		"gpt-codex": {
			id: "gpt-codex",
			name: "GPT Codex",
...
describe("OpenCode provider response sanitizer", () => {
	test("allowlists the provider catalog before it reaches Renderer state", () => {
		...
		const model = (sanitized.models as Record<string, Record<string, unknown>>)["gpt-codex"]
	...
	test("leaves unrelated and unsuccessful responses unchanged", () => {
		...
		expect(sanitizeOpenCodeProviderResponse("http://127.0.0.1:4101/session", 200, body)).toBe(
			body,
		)
		expect(sanitizeOpenCodeProviderResponse("http://127.0.0.1:4101/provider", 500, body)).toBe(
			body,
		)
	})
	...
	test("fails closed for malformed successful provider responses", () => {
		expect(() =>
			sanitizeOpenCodeProviderResponse("http://127.0.0.1:4101/provider", 200, "not-json"),
