...
export function buildImageRef(repository: GitHubRepositoryRef, headSha: string): string {
...
export function parseGitHubBuildArtifact(
	...
	if (!value || typeof value !== "object") throw new Error("GitHub build artifact is invalid")
	...
	if (commit !== headSha.toLowerCase() || tag !== buildImageRef(repository, headSha)) {
		throw new Error("GitHub build artifact does not match the requested commit")
	}
	if (
		!new RegExp(
...
export function isSensitiveProjectPath(projectPath: string): boolean {
	...
	if (segments.includes(".local") || segments.includes("node_modules")) return true
	if (/(^|\/)\.sealos\/(?:logs|auth(?:\.json)?)(?:\/|$)/.test(normalized)) return true
	if (basename === "kubeconfig" || basename === "id_rsa" || basename === "id_ed25519") return true
	if (
		basename === ".env" ||
