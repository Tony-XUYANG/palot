...
import {
	...
	probeOfficialCodexCli,
} from "./codex-cli-probe.ts";
...
function options(includeRuntime: boolean) {
	return {
		...
		arch: "x64",
		environment: includeRuntime
			? { PATH: "", PALOT_TEST_CODEX_PATH: codexPath }
			: { PATH: "" },
		fileExists: (candidate: string) =>
			includeRuntime && candidate === codexPath,
	};
}
	...
	it("detects a structured app-server without enabling it", async () => {
		...
		const runner: CodexCommandRunner = async (_filePath, args) => {
			...
			return {
				ok: true,
				stdout:
					args[0] === "--version" ? "codex-cli 0.146.0" : "app-server options",
				stderr: "",
			};
		};
