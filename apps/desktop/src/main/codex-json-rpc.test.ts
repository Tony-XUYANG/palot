...
import { CodexJsonRpcClient, type CodexJsonRpcProcess } from "./codex-json-rpc.ts"
...
function createProcess(): {
	...
	const process = Object.assign(emitter, { stdin, stdout }) as CodexJsonRpcProcess
	return { process, stdin, stdout, emitter }
}

async function readRequest(stdin: PassThrough): Promise<Record<string, unknown>> {
...
describe("Codex newline JSON-RPC transport", () => {
	it("frames partial lines and correlates concurrent responses", async () => {
		...
		const firstResponse = JSON.stringify({ id: firstRequest.id, result: { value: "one" } })
	...
	it("removes timed-out and exited requests", async () => {
		...
		const exitClient = new CodexJsonRpcClient({ process: exitTransport.process })
