...
async function computeNextRun(
	...
	try {
		...
		if (next) {
			...
			log.warn("RRULE produced no future occurrence", {
				id,
				rruleStr,
				now: now.toISOString(),
			});
...
async function scheduleNext(
	...
	task.timer = setTimeout(async () => {
		...
		log.info("Timer fired, starting automation callback", {
			id,
		...
		try {
			...
			if (tasks.get(id) === task && !task.paused) {
				...
				try {
					...
					log.error(
						"Failed to reschedule automation after run -- timer chain broken",
						{ id },
						err,
					);
				}
