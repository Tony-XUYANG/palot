			...
			for await (const event of result.stream) {
				...
				if (evt.type === "session.error") {
					if (
						...
						const errMsg =
							errObj.data?.message ?? errObj.name ?? "Unknown error";
				...
				if (evt.type === "question.asked") {
					if (evt.properties?.sessionID === sessionId) {
						log.warn("Auto-rejecting question during automation", {
							sessionId,
						});
						try {
...
export async function executeRun(
	...
	try {
		...
		const permissionRuleset = buildPermissionRuleset(
			config.execution.permissionPreset ?? "default",
		);

		// If running in a worktree, create a client scoped to that directory
		const sessionClient = worktreePath
			? (createAutomationClient(worktreePath) ?? client)
			: client;
		...
		const model = config.execution.model
			? parseModelRef(config.execution.model)
			: undefined;
		...
		try {
			const sessionInfo = await sessionClient.session.get({
				sessionID: sessionId,
			});
