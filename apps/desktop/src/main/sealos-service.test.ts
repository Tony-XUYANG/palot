...
describe("Sealos template helpers", () => {
	...
	it("rejects mutable image references", async () => {
		...
		await assert.rejects(
			updateSealosTemplateImage(root, "ghcr.io/owner/demo:latest"),
		);
	});
	...
	it("requires stable pod identity and restart counts across the runtime window", () => {
		...
		assert.equal(
			isSealosClusterStable(baseline, { ...baseline, podUids: ["pod-b"] }),
			false,
		);
	});
	...
	it("retries only transient kubectl transport failures", () => {
		assert.equal(
			isTransientKubectlError({
				stderr: "Unable to connect: net/http: TLS handshake timeout",
			}),
			true,
		);
		assert.equal(
			isTransientKubectlError(new Error("connection reset by peer")),
			true,
		);
		assert.equal(
			isTransientKubectlError({ stderr: "Error from server (Forbidden)" }),
			false,
		);
	});
	...
	it("retries only transient HTTP transport failures", () => {
		...
		assert.equal(
			isTransientFetchError(new Error("socket closed during TLS handshake")),
			true,
		);
		assert.equal(
			isTransientFetchError(new Error("net::ERR_CONNECTION_CLOSED")),
			true,
		);
		assert.equal(
			isTransientFetchError(new Error("Sealos returned HTTP 403")),
			false,
		);
	});
});
