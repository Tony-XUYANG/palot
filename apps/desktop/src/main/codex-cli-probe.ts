...
import { resolveCodexRuntime, type RuntimeResolverOptions } from "./runtime-resolver"
...
async function runCommand(filePath: string, args: string[]): Promise<CommandResult> {
...
async function executeProbeCommand(
	...
	try {
		...
		return {
			ok: false,
			stdout: "",
			stderr: error instanceof Error ? error.message : "The command could not be executed.",
		}
	}
}
...
export async function probeOfficialCodexCli(
	...
	const versionResult = await executeProbeCommand(runner, runtime.path, ["--version"])
	if (!versionResult.ok) {
		return {
			id: "codex",
			label: "Official Codex CLI",
	...
	if (!appServerResult.ok) {
		return {
			...
			reason: "The detected Codex CLI does not expose the required app-server interface.",
		}
	}
	...
	return {
		...
		reason: "Detected for compatibility testing; the official engine is not enabled yet.",
	}
}
