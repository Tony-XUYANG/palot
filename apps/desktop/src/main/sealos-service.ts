...
function getNamespaces(payload: Record<string, unknown>): Record<string, unknown>[] {
...
export async function listSealosWorkspaces(): Promise<SealosWorkspace[]> {
	...
	const payload = await fetchJson(`${normalizeRegion(auth.region)}/api/auth/namespace/list`, {
...
export async function switchSealosWorkspace(workspaceId: string): Promise<SealosWorkspace> {
	const auth = await readStoredAuth()
	if (!auth.region || !auth.regional_token) throw new Error("Sign in to Sealos first")
	...
	const switched = await fetchJson(`${region}/api/auth/namespace/switch`, {
		method: "POST",
		headers: { Authorization: auth.regional_token, "Content-Type": "application/json" },
		body: JSON.stringify({ ns_uid: workspace.uid }),
	})
	const regionalToken = readNestedString(switched, ["data", "token"])
	if (!regionalToken) throw new Error("Sealos workspace switch did not return a regional token")
	const kubeconfigPayload = await fetchJson(`${region}/api/auth/getKubeconfig`, {
		headers: { Authorization: regionalToken },
	})
	const kubeconfig = readNestedString(kubeconfigPayload, ["data", "kubeconfig"])
	if (!kubeconfig) throw new Error("Sealos workspace did not return a kubeconfig")
	...
	await Promise.all([
		writeFile(path.join(sealosDirectory, "auth.json"), JSON.stringify(auth, null, 2), {
			mode: 0o600,
		}),
	...
	try {
		while (Date.now() < login.expiresAt) {
			...
			try {
				...
				const candidates = (workspaceData.data as { namespaces?: unknown[] } | undefined)
					?.namespaces
				const selected =
					candidates?.find((item) => (item as { nstype?: string }).nstype === "private") ??
					candidates?.[0]
...
export async function readSealosTemplateInputs(directory: string): Promise<SealosTemplateInput[]> {
	...
	for (const document of documents) {
		...
		return Object.entries(value.spec.inputs).map(([name, raw]) => {
			...
			return {
				...
				defaultValue: input.default === undefined ? null : String(input.default),
...
export async function updateSealosTemplateImage(directory: string, image: string): Promise<void> {
	...
	for (const document of documents) {
		...
		document.setIn(["spec", "template", "spec", "containers", 0, "image"], image)
		updated = true
		break
	}
...
export async function readSealosDeploymentState(
	...
	try {
		return JSON.parse(await readFile(getStatePath(directory), "utf8")) as SealosDeploymentState
	} catch (error) {
...
function getKubectlPath(): string {
	...
	const runtime = resolveKubectlRuntime({
		isPackaged: Boolean(electronProcess.resourcesPath && !electronProcess.defaultApp),
		resourcesPath: electronProcess.resourcesPath,
	})
	if (!runtime) throw new Error("kubectl is unavailable. Reinstall Palot to repair the runtime.")
	return runtime.path
}
...
interface KubernetesPod {
	...
	status?: {
		...
		initContainerStatuses?: { name?: string; ready?: boolean; restartCount?: number }[]
	}
}
...
function createLabelSelector(labels: Record<string, string> | undefined): string | null {
...
async function countSevereLogs(pods: KubernetesPod[], namespace: string): Promise<number> {
	...
	if (results.some((result) => result.status === "rejected")) return Number.POSITIVE_INFINITY
	let count = 0
...
async function readSealosClusterSnapshot(
	...
	const deployment = parseKubectlJson<KubernetesDeployment>(
		await kubectl(["get", `deployment/${appName}`, "-n", namespace, "-o", "json"]),
	)
	...
	const [podsPayload, endpointsPayload, ingressesPayload] = await Promise.all([
		kubectl(["get", "pods", "-n", namespace, ...(selector ? ["-l", selector] : []), "-o", "json"]),
		kubectl(["get", "endpoints", "-n", namespace, "-o", "json"]),
		kubectl(["get", "ingress", "-n", namespace, "-o", "json"]),
	])
	const pods = parseKubectlJson<KubernetesList<KubernetesPod>>(podsPayload).items ?? []
	const endpoints =
		parseKubectlJson<KubernetesList<KubernetesEndpoints>>(endpointsPayload).items ?? []
	const ingresses =
		parseKubectlJson<KubernetesList<KubernetesIngress>>(ingressesPayload).items ?? []
	...
	for (const rule of ingress?.spec?.rules ?? []) {
		for (const ingressPath of rule.http?.paths ?? []) {
			...
			if (service?.name) publicBackends.set(service.name, service.port?.number ?? null)
		}
	}
	const publicEndpoints = endpoints.filter((candidate) =>
		candidate.metadata?.name ? publicBackends.has(candidate.metadata.name) : false,
	)
	...
	const podUids = pods.flatMap((pod) => (pod.metadata?.uid ? [pod.metadata.uid] : [])).sort()
	...
	for (const pod of pods) {
		...
		for (const container of [
			...
			if (pod.metadata?.uid && container.name) {
				restarts[`${pod.metadata.uid}/${container.name}`] = container.restartCount ?? 0
			}
		}
	}
	...
	return {
		snapshot: {
			...
			endpointsReady:
				...
				publicEndpoints.every((candidate) => countReadyAddresses(candidate) > 0),
...
export function isSealosClusterStable(
	...
	const restartKeys = new Set([...Object.keys(baseline.restarts), ...Object.keys(final.restarts)])
	for (const key of restartKeys) {
		if ((baseline.restarts[key] ?? 0) !== (final.restarts[key] ?? 0)) return false
	}
	return true
}
...
async function verifyLaunchpadNetwork(
	...
	const matching = networks.find((network) => {
		...
		return hosts.includes(host) && (expectedPort === null || network.port === expectedPort)
	})
...
export async function updateSealosDeployment(directory: string): Promise<SealosUpdateResult> {
	const state = await readSealosDeploymentState(directory)
	if (!state) throw new Error("No previous Sealos deployment was found for this project")
	const templatePath = path.join(path.resolve(directory), ".sealos", "template", "index.yaml")
	const image = getTemplateImage(await readFile(templatePath, "utf8"))
	if (!image) throw new Error("The Sealos template does not contain an application image")
	const { app_name: appName, namespace, image: previousImage } = state.last_deploy
	const deploymentJson = JSON.parse(
		await kubectl(["get", `deployment/${appName}`, "-n", namespace, "-o", "json"]),
	) as { spec?: { template?: { spec?: { containers?: { name?: string }[] } } } }
	const container = deploymentJson.spec?.template?.spec?.containers?.[0]?.name
	if (!container) throw new Error("The existing Sealos deployment has no application container")
	...
	try {
		...
		await kubectl(["rollout", "status", `deployment/${appName}`, "-n", namespace, "--timeout=180s"])
		...
		return { success: true, appName, image, previousImage, url: state.last_deploy.url }
	} catch (error) {
		await kubectl(["rollout", "undo", `deployment/${appName}`, "-n", namespace]).catch(() => "")
		state.history.push({
			at: now,
			action: "set-image",
			status: "failed",
			...
			image,
			previous_image: previousImage,
			note: error instanceof Error ? error.message : "Rollout failed and was rolled back",
		})
		await writeSealosDeploymentState(directory, state)
		throw error
	}
...
async function postTemplate(
	...
	if (!response.ok)
		throw new Error(`Sealos deployment failed (${response.status}): ${responseText}`)
...
function findInstanceName(value: unknown): string | null {
	...
	if (record.resourceType === "instance" && typeof record.name === "string") return record.name
...
export async function deployToSealos(
	...
	if (await readSealosDeploymentState(directory)) {
		throw new Error("This project already has a Sealos deployment. Use Update deployment instead.")
	}
	const sealosDirectory = path.join(os.homedir(), ".sealos")
	...
	try {
		...
		if (instanceName) {
			...
			if (!image) throw new Error("Deployment succeeded but its image could not be recorded")
			const now = new Date().toISOString()
...
export async function verifySealosRuntime(
	...
	try {
		const state = await readSealosDeploymentState(directory)
		if (!state) throw new Error("No Sealos deployment state was found for this project")
		await kubectl([
			"rollout",
			"status",
		...
		const clusterStable = isSealosClusterStable(baseline.snapshot, final.snapshot)
		const checks: SealosRuntimeResult["checks"] = [
			...
			{
				id: "logs",
				ok: Number.isFinite(final.snapshot.severeLogCount) && final.snapshot.severeLogCount === 0,
				detail:
		...
		return {
			...
			detail: acceptable ? "Public runtime checks passed" : "One or more runtime checks failed",
			checks,
		}
	} catch (error) {
		return {
