...
export async function ensureServer(): Promise<OpenCodeServer> {
	...
	if (detection.kind === "found") {
		log.info("Detected existing same-user server", {
			url: detection.server.url,
		});
...
async function handleLockfile(
	...
	if (await probeServer(url)) {
		log.info("Reconnecting to server from lockfile", {
			url,
			pid: lockfile.pid,
		});
...
async function detectExistingServer(
	...
	if (!owner) {
		...
		if (process.platform === "win32") {
			log.debug(
				"Existing server responded OK (ownership check skipped on Windows)",
				{ url },
			);
	...
	if (isCurrentUser(owner.uid)) {
		log.debug("Existing server belongs to current user", {
			url,
			pid: owner.pid,
			uid: owner.uid,
		});
	...
	log.warn("Existing server belongs to a DIFFERENT user", {
		url,
		pid: owner.pid,
		uid: owner.uid,
	});
...
async function handleConflict(
	...
	const ownerText =
		conflict.ownerUid !== null
			? tMain("native.dialogs.foreignOwner", { uid: conflict.ownerUid })
			: tMain("native.dialogs.ownerUnknown");

	const { response } = await dialog.showMessageBox({
		type: "warning",
		title: tMain("native.dialogs.serverOwnershipConflict"),
		message: tMain("native.dialogs.serverAlreadyRunning"),
		detail: `${ownerText}\n\n` + tMain("native.dialogs.foreignServerWarning"),
		buttons: [
			tMain("native.dialogs.startOwnServer"),
			tMain("native.dialogs.connectAnyway"),
			tMain("common.actions.cancel"),
		],
		defaultId: 0,
		cancelId: 2,
	});
	...
	if (response === 1) {
		...
		log.warn("User chose to connect to foreign server anyway", {
			url: conflict.url,
		});
	...
	// Cancel
	throw new Error(
		"Server connection cancelled by user due to ownership conflict",
	);
}
...
async function probeServer(url: string): Promise<boolean> {
	try {
		...
		log.debug("Server probe returned error status", {
			url,
			status: res.status,
		});
	} catch (err) {
