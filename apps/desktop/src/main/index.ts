...
if (process.platform === "linux") {
	...
	if (
		...
		try {
			...
			const match = dbusOutput.match(
				/\(\d+,\s*\d+,\s*([\d.]+),\s*uint32\s+\d+,\s*true/,
			);
			if (match) {
				...
				if (scale > 0 && scale !== Math.floor(scale)) {
					...
					log.info(
						`Wayland fractional scale detected (${scale}), forcing device scale factor`,
					);
				}
			}
		} catch {
...
if (!gotLock) {
	...
	app.whenReady().then(() => {
		...
		initSettingsStore();
		setMainLocale(getSettings().language ?? "en-US");
		onSettingsChanged((settings) =>
			setMainLocale(settings.language ?? "en-US"),
		);
		initCredentialStore();
		registerIpcHandlers();
		initAutomations().catch(console.error);
		startMdnsScanner().catch((err) =>
			log.warn("mDNS scanner failed to start", err),
		);
		createWindow();
