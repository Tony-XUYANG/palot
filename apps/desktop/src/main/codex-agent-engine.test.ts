...
import {
	createCodexAgentEngine,
	parseCodexUnifiedDiff,
} from "./codex-agent-engine.ts";
...
function createMockServer() {
	...
	stdin.on("data", (chunk: string) => {
		...
		for (;;) {
			...
			stdout.write(
				`${JSON.stringify({ id: request.id, result: responseFor(request.method) })}\n`,
			);
		}
	});
...
describe("official Codex app-server preview", () => {
	it("maps auth, models, sessions, prompts, diff events, and cancellation", async () => {
		...
		const turnRequest = server.requests.find(
		...
		assert.equal(
			(await engine.diff("C:\\project", session.id))[0]?.file,
			"health.ts",
		);
	...
	it("starts a fresh app-server after an unexpected process exit", async () => {
		...
		let spawnCount = 0;
		const spawnProcess = (() =>
			servers[spawnCount++].child) as unknown as typeof spawn;
		const engine = createCodexAgentEngine({
			runtimePath: "C:\\runtime\\codex.exe",
			codexHome: "C:\\PalotData\\codex",
			inspect: async () => ({
