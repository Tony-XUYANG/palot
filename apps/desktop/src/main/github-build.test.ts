...
describe("GitHub Actions remote build helpers", () => {
	it("parses HTTPS and SSH GitHub remotes", () => {
		assert.equal(
			parseGitHubRemote("https://github.com/Tony-XUYANG/demo.git")
				?.nameWithOwner,
			"Tony-XUYANG/demo",
		);
		assert.equal(
			parseGitHubRemote("git@github.com:owner/app.git")?.nameWithOwner,
			"owner/app",
		);
	...
	it("invalidates resumed builds when source or HEAD changed", () => {
		...
		assert.equal(
			isGitHubBuildCurrent(build, "main", build.commit, [
				".sealos/template/index.yaml",
			]),
			true,
		);
		assert.equal(
			isGitHubBuildCurrent(build, "feature", build.commit, []),
			false,
		);
		assert.equal(
			isGitHubBuildCurrent(build, "main", "f".repeat(40), []),
			false,
		);
		assert.equal(
			isGitHubBuildCurrent(build, "main", build.commit, ["src/index.ts"]),
			false,
		);
	});
