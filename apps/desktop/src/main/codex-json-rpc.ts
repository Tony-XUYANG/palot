...
export class CodexJsonRpcClient {
	...
	private readonly onNotification?: (notification: CodexJsonRpcNotification) => void
	private readonly onClose?: (error: Error) => void
	private readonly notificationQueue = new AsyncEventQueue<CodexJsonRpcNotification>()
	...
	private handleResponse(response: JsonRpcResponse): void {
		...
		if (response.error) {
			request.reject(
				new Error(response.error.message ?? `Codex JSON-RPC error ${response.error.code ?? "unknown"}`),
			)
			return
		}
