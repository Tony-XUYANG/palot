...
import { onMainLocaleChanged, tMain } from "./i18n";
...
export function createTray(
	...
	if (IS_MAC) {
		...
		if (!fs.existsSync(templatePath)) {
			log.error(
				`Tray icon not found at ${templatePath} — tray will be invisible`,
			);
		}
		...
		if (!fs.existsSync(trayIconPath)) {
			log.warn(
				`Tray icon not found at ${trayIconPath} — falling back to icon.png`,
			);
		}
	...
	unsubscribeLocale = onMainLocaleChanged(() => rebuildMenu());
	...
	log.info(
		`Tray created (platform: ${IS_MAC ? "macOS" : IS_LINUX ? "Linux" : "Windows"})`,
	);
}
...
function rebuildMenu(): void {
	...
	if (pendingCount > 0) {
		template.push({
			label: `⚠ ${tMain(
				pendingCount === 1
					? "native.tray.pendingApproval"
					: "native.tray.pendingApprovals",
				{ count: pendingCount },
			)}`,
			enabled: true,
			click: () => showWindow(),
	...
	if (serverUrl) {
		template.push({
			label: tMain("native.tray.serverRunning"),
			enabled: false,
		});
	}
...
function buildAgentSection(
	...
	for (const [sessionId, state] of liveSessions) {
		...
		const dir = state.directory || tMain("native.tray.unknown");
		...
		group.agents.push({
			sessionId,
			title: state.title || tMain("native.tray.untitled"),
			status: state.status,
		});
	}
	...
	const sortedGroups = Array.from(groups.values()).sort((a, b) => {
		const aBusy = a.agents.filter(
			(a) => a.status === "busy" || a.status === "retry",
		).length;
		const bBusy = b.agents.filter(
			(a) => a.status === "busy" || a.status === "retry",
		).length;
	...
	const headerLabel =
		totalBusy > 0
			? tMain("native.tray.agentsRunning", { count: totalBusy })
			: tMain("native.tray.agentSessions", { count: liveSessions.size });
	...
	for (const group of sortedGroups) {
		...
		if (sortedGroups.length > 1) {
			...
			const projectLabel =
				busyInGroup > 0
					? `${group.name}  (${tMain("native.tray.running", { count: busyInGroup })})`
					: group.name;
...
function buildAgentItems(
	...
	if (overflow.length > 0) {
		items.push({
			label: tMain("native.tray.viewMore", { count: overflow.length }),
...
function agentMenuItem(agent: {
	...
	const maxLen = 40;
	const title =
		agent.title.length > maxLen
			? `${agent.title.slice(0, maxLen)}...`
			: agent.title;
...
function buildRecentSection(
	...
	if (recentSessions.length > 5) {
		items.push({
			label: tMain("native.tray.viewMore", { count: recentSessions.length }),
			click: () => showWindow(),
		});
	}
...
function updateTrayTitle(
	...
	const totalSessions = Array.from(liveSessions.values()).filter(
		(s) => !s.parentID,
	).length;
	const busyCount = Array.from(liveSessions.values()).filter(
		(s) => !s.parentID && (s.status === "busy" || s.status === "retry"),
	...
	let tooltip = "Palot";
	if (totalSessions > 0) {
		tooltip += ` - ${tMain("native.tray.agentSessions", { count: totalSessions })}`;
		if (busyCount > 0) {
			tooltip += ` (${tMain("native.tray.running", { count: busyCount })})`;
		}
	}
	if (pendingCount > 0) {
		tooltip += ` - ${tMain("native.tray.pending", { count: pendingCount })}`;
	}
...
function formatRelativeTime(timestampMs: number): string {
	...
	if (seconds < 60) return tMain("native.tray.now");
	const minutes = Math.floor(seconds / 60);
	if (minutes < 60) return tMain("native.tray.minutesAgo", { count: minutes });
	const hours = Math.floor(minutes / 60);
	if (hours < 24) return tMain("native.tray.hoursAgo", { count: hours });
	const days = Math.floor(hours / 24);
	if (days < 30) return tMain("native.tray.daysAgo", { count: days });
	const months = Math.floor(days / 30);
	return tMain("native.tray.monthsAgo", { count: months });
}
