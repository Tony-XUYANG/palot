...
function mapSession(thread: CodexThreadStartResponse["thread"], fallbackTitle?: string): AgentSession {
	return {
		id: thread.id,
...
function mapPromptPart(part: AgentPromptPart): CodexUserInput {
	...
	if (!part.mime.startsWith("image/")) {
		throw new Error("The official Codex preview currently accepts only text and image prompt parts")
	}
...
export function createCodexAgentEngine(dependencies: CodexAgentEngineDependencies): CodexAgentEngine {
	...
	async function getRpc(): Promise<CodexJsonRpcClient> {
		...
		process = spawnProcess(
			...
			{
				env: { ...(dependencies.environment ?? globalThis.process.env), CODEX_HOME: dependencies.codexHome },
				stdio: "pipe",
				windowsHide: true,
			},
		)
		...
		try {
			...
			client.dispose(error instanceof Error ? error : new Error("Codex initialization failed"))
			failedProcess?.kill()
			throw error
		}
	}
	...
	return {
		...
		async authStatus() {
			...
			const result = await client.request<CodexAccountReadResponse>("account/read", {
				refreshToken: false,
			})
			return {
		...
		async beginLogin(): Promise<AgentEngineLoginResult> {
			const client = await getRpc()
			const result = await client.request<CodexLoginResponse>("account/login/start", {
				type: "chatgpt",
				appBrand: "codex",
		...
		async listModels(): Promise<AgentEngineModel[]> {
			...
			do {
				const result: CodexModelListResponse = await client.request<CodexModelListResponse>(
					"model/list",
					{
					cursor,
					includeHidden: false,
		...
		async createSession(directory: string, title?: string): Promise<AgentSession> {
			const client = await getRpc()
			const result = await client.request<CodexThreadStartResponse>("thread/start", {
				cwd: directory,
		...
		async prompt(request: AgentEnginePromptRequest): Promise<void> {
			const client = await getRpc()
			const result = await client.request<CodexTurnStartResponse>("turn/start", {
		...
		async cancel(_directory: string, sessionID: string): Promise<void> {
			...
			if (!turnID) throw new Error("The Codex session has no active turn to cancel")
			const client = await getRpc()
			await client.request("turn/interrupt", { threadId: sessionID, turnId: turnID })
			activeTurns.delete(sessionID)
		},
		async diff(_directory: string, sessionID: string): Promise<AgentFileDiff[]> {
